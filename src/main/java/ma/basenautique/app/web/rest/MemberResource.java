...
/**
 * REST controller for managing {@link ma.basenautique.app.domain.Member}.
 */
@RestController
@RequestMapping("/api")
public class MemberResource {
     ...
     */
    @GetMapping("/members")
    public ResponseEntity<List<MemberDTO>> getAllMembers(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("child-is-null".equals(filter)) {
            log.debug("REST request to get all Members where child is null");
            return new ResponseEntity<>(memberService.findAllWhereChildIsNull(),
                    HttpStatus.OK);
        }
