...
export class TypeDocComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.typeDocService
        ...
        .subscribe(
          (res: HttpResponse<ITypeDoc[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInTypeDocs(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeDocListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeDoc[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/type-doc'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
