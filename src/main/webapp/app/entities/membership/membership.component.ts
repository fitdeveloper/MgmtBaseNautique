...
export class MembershipComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.membershipService
        ...
        .subscribe(
          (res: HttpResponse<IMembership[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInMemberships(): void {
    this.eventSubscriber = this.eventManager.subscribe('membershipListModification', () => this.loadPage());
  }
